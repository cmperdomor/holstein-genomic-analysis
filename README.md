...
### 8. Which apparent trait associations are structurally redundant? (notebook 03)

**For the producer.** Some evaluations move together for arithmetic reasons rather than biology, so tracking every pairwise correlation adds limited information.

**Statistical detail.** Within the production block, evaluations are highly collinear (VIF up to 961 for %Fat), because component percentages are mathematical functions of component yields and milk volume. This demonstrates structural multicollinearity within the production block; it is not a statement about conformation traits, which were not analysed for redundancy in this project. The dimensionality of the six functional subindexes is addressed separately in question 4 (notebook 05) and is not repeated here.
...
Pedigree and genomic estimates disagree substantially at the animal level (limits of agreement [-1.33, 8.69]), and at the 8% threshold 210 females move from below on pedigree to at-or-above on genomic, with 2 moving the opposite way (212 discordant of 489). Both are valid estimates of different quantities; in total 212 of 489 paired females are discordant at the 8% threshold. The genomic measure is already available in this herd's export, so no additional testing is required to use it in the current mating workflow.
...
Genomic evaluations are constructed values, and several relationships in this dataset are arithmetic rather than biological. The analysis was built to separate them: composite traits were excluded from correlation matrices containing their own components; the profit result was interpreted cautiously (the SIZE-Pro$ association may partly reflect economic relationships between maintenance requirements and predicted profitability, but the unpublished Pro$ formula prevents determining how much of the association is structural); mechanisms were tested formally rather than assumed (cluster-bootstrap mediation shows BMR does not account for the size-fat association, indirect effect CI [-0.326, +0.771] includes zero). Sire-clustered standard errors, effect sizes, confidence intervals, equivalence testing, minimum detectable effects and FDR correction were used where relevant to the corresponding statistical question, not in every analysis.
...
* Lactanet, Pro$: detailed formula is not published, so the extent to which size-related associations with Pro$ are structural cannot be determined
* Lactanet, Modernized Holstein LPI (April 2025): Reproduction Index 10% of LPI; Daughter Fertility 90% of the Reproduction Index (about 9% of total direct LPI weighting)
...
* Oliveira Junior et al. (2021), J. Dairy Sci.: genetic parameters for Canadian Holstein traits; fertility traits are low-heritability; in the modernized Holstein LPI, Daughter Fertility corresponds to about 9% of total direct LPI weighting (Reproduction Index 10% times Daughter Fertility 90%). (The previously cited "Timed AI, R=0.5, sire re-ranking" claim is not supported by this source and has been removed.)
